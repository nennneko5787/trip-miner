...
/// シェーダ原文の埋め込み(実行時コンパイル用)。
/// 対応する .metal ファイルを編集した場合は、このファイルも scripts/gen-shaders 相当の手順で再生成すること。
/// (.metal はXcodeのターゲットに含めない。含めるとダミーmatcherのままコンパイルされるだけになるため)
enum Shaders {
    static let crypt: String = """
